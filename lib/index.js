...
const app = new Koa(),
    router = new Router(),
    urlGenerator = new UrlGenerator(`${config.scheme}://${config.host}:${config.public_port}`, router),
    ...
    appController = new AppController(appFinder),
    releaseController = new ReleaseController(appFinder, releaseFinder, urlGenerator),
    updateController = new UpdateController(appFinder, releaseFinder, urlGenerator, downloader);
